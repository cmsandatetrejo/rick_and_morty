...
const character ={id:923,name:'Dai',species:'Human',gender:'Female',status:'Alive',origin:{name:'Earth 137'},image:'image.jpg'};
...
describe("Test de RUTAS", ()=>{
    describe("GET /rickandmorty/character/:id", ()=>{
        ...
        it("Si hay un error responde con status: 500", async () => {
            const response=await request.get('/rickandmorty/character/5241');
    ...
    describe("GET /rickandmorty/login", ()=>{
        ...
        it("Responde con un objeto con la propiedad access en true si la informacion del usuario es valida", async ()=>{
            const response =await request.get('/rickandmorty/login?email=sand6@hotmail.com&password=123asd');
            expect(response.body).toEqual(access);
        })

        it("Responde con un objeto con la propiedad access en false si la informacion del usuario no es valida", async ()=>{
            const response =await request.get('/rickandmorty/login?email=sand6@hotmail.com&password=123klñ');
            access.acces=false;
            expect(response.body).toEqual(access);
        })
    })
